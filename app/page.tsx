...
export default function Home() {
  ...
  const categories = ['Semua', 'Material', 'Alat Konstruksi', 'Interior', 'Elektronik', 'Lainnya']

  useEffect(() => {
    fetchListings()
  }, [activeCategory]) // Refetch otomatis saat klik kategori
    ...
    <div className="bg-gray-50 min-h-screen flex flex-col font-sans">
      ...
      <nav className="bg-white/95 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-24 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-4 group">
            <img src="/images/logo-tokosecond.png" alt="Logo" className="h-12 w-auto object-contain transition-transform group-hover:scale-105" />
          ...
          <Link href="/jual" suppressHydrationWarning className="bg-[#EE4D2D] hover:bg-[#d73d1f] text-white text-[11px] font-black px-8 py-3.5 rounded-full flex items-center gap-2 shadow-xl shadow-orange-100 transition-all transform active:scale-95 tracking-widest uppercase">
      ...
      <main className="max-w-7xl mx-auto px-4 py-10 flex-grow">
        ...
        <div className="text-center mb-10">
          <h2 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tighter uppercase mb-3 italic">
            "Dari yang terpakai, menjadi bernilai & berkah"
          </h2>
        </div>

        {/* FILTER BOX (FIXED) */}
        <div className="bg-white p-8 rounded-[3rem] shadow-2xl shadow-gray-200/50 border border-white mb-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input suppressHydrationWarning placeholder="Harga Min (Rp)" type="number" className="w-full bg-gray-50 border-none p-4 rounded-2xl focus:ring-2 focus:ring-[#EE4D2D] outline-none text-xs font-bold transition-all" onChange={(e) => setMinPrice(e.target.value)} />
            <input suppressHydrationWarning placeholder="Harga Max (Rp)" type="number" className="w-full bg-gray-50 border-none p-4 rounded-2xl focus:ring-2 focus:ring-[#EE4D2D] outline-none text-xs font-bold transition-all" onChange={(e) => setMaxPrice(e.target.value)} />
            <input suppressHydrationWarning placeholder="Cari Kota..." className="w-full bg-gray-50 border-none p-4 rounded-2xl focus:ring-2 focus:ring-[#EE4D2D] outline-none text-xs font-bold transition-all" onChange={(e) => setFilterCity(e.target.value)} />
            <button suppressHydrationWarning onClick={fetchListings} className="bg-[#EE4D2D] hover:bg-orange-600 text-white font-black p-4 rounded-2xl transition-all active:scale-95 uppercase text-[10px] tracking-widest shadow-lg shadow-orange-100">
              Terapkan Filter
            </button>
          </div>
        </div>
            ...
            {listings.map((item) => (
              <Link key={item.id} href={`/listing/${item.id}`} className="group">
                <div className="bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-3">
                  <div className="aspect-square relative overflow-hidden bg-gray-50 text-center">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className="bg-white/90 backdrop-blur px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-tighter border border-white">📍 {item.location_city}</span>
                    </div>
                  </div>
                  <div className="p-6">
                    ...
                    <p className="text-[9px] font-bold text-gray-400 uppercase mt-3 tracking-widest italic opacity-0 group-hover:opacity-100 transition-opacity">📦 {item.category}</p>
                  </div>
                </div>
              </Link>
            ))}
      ...
      <footer className="bg-white border-t-4 border-[#EE4D2D] py-16 mt-20">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-gray-500 leading-relaxed uppercase tracking-tighter mb-4">[Dari yang terpakai, menjadi bernilai & berkah]</p>
            <p className="text-[10px] font-medium text-gray-400">Menghubungkan kebaikan melalui barang yang masih layak manfaat.</p>
          </div>
